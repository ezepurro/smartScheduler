...
const AdminPage = () => {
    ...
    return (
        <div className="flex h-screen">
            {/* Sidebar */}
            <aside
                className={`bg-brand-primary text-brand-secondary w-64 md:w-1/5 p-4 absolute md:static inset-y-0 left-0 transform ${isOpen ? "translate-x-0" : "-translate-x-full"
                    ...
                    {adminSections.map((section) => (
                        <div key={section.id}>
                            <button
                                ...
                                className="block w-full text-left border-b border-brand-secondary pb-2 cursor-pointer"
                            >
                                {section.title}
                            </button>
                        </div>
            ...
            {/* Main Content */}
            <main className="flex-1 p-6 flex items-center justify-center md:pl-1/5">
                <SelectedComponent />
            </main>
        </div>
    );
